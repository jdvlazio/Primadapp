...
2. **REPARTO — a QUIÉNES distribuye el Tesorero** (`.bal-stat` cabecera "Reparto a ahorradores · N" + `.bal-stat-v`
   "$X c/u" teal + `.bal-rep-list`): la lista **nombrada de los ahorradores** que reciben `parteIgual`. El **ANFITRIÓN
   va en la lista, marcado `.bal-rep-anf` "Anfitrión"** (siempre es ahorrador → también recibe); los **invitados NO**
   (generan ganancia, no la cobran). Es **distinto del COBRO** (quién paga): esto es quién COBRA del fondo.
   **§0 — el monto va UNA vez en la cabecera** (`$X c/u`), **NO por fila** (es igual para todos: la lista solo aporta
   los NOMBRES, el dato nuevo). Sin nombrar a los beneficiarios, el Tesorero no sabría a quién entregarle.
...
| `.bal-stat` | cabecera del REPARTO: `.bal-stat-k` "Reparto a ahorradores" + `.bal-stat-sub` "N ahorradores" + `.bal-stat-v` "$X c/u" (teal, el monto UNA vez). Borde-top tenue lo separa del héroe |
| `.bal-rep-list` / `.bal-rep` | lista nombrada de beneficiarios (ahorradores): **solo el nombre** (anfitrión + `.bal-rep-anf` "Anfitrión" teal). **Sin monto por fila** (§0: ya está en la cabecera). Anfitrión primero, luego por nombre |
